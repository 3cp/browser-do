...
test('browser-do', async t => {
  await t.test('browser-do rans javascript code', async t => {
  ...
  await t.test('browser-do rans javascript, close by api', async t => {
    return new Promise(resolve => {
      const browser = run();
      browser.pipe(concat(data => {
        t.equal(data.toString(), 'hello\n');
  ...
  await t.test('browser-do detects tap output, auto close', async t => {
    function onCoverage() {
  ...
  await t.test('browser-do detects tap output, auto close, write coverage report', async t => {
